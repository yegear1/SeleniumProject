...
from utils import normalize_gpu_name, normalize_price, human_scroll
...
def scrape_pichau(driver, current_date, site="pichau", counters=None):
    ...
    try:
        ...
        logger.error(f"Erro ao coletar o número de paginas: {e}")
        page_count = 1
    ...
    while current_page <= page_count:
        ...
        for grid in products_grid:
            try:
                ...
                try:
                    ...
                    logger.info("Erro ao coletar o preco")
                    continue
                ...
                print(f"Erro ao processar produto na pagina {current_page}: {e}")
                continue
        ...
        if current_page < page_count:
            try:
                ...
                if human_scroll(driver, next_button):
                    ActionChains(driver).move_to_element(next_button).click().perform()
                else:
                    logger.warning("Scroll falhou, tentando clique direto")
                    next_button.click()
                    
                current_page += 1
            except Exception as e:
                logger.error(f"Erro ao passar para a próxima pagina: {e}")
                break
        else:
            break
